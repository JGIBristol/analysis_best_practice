...
git branch -a
```
* Change to the your workshop-practice branch using the following command:
```markdown
git checkout --track remotes/origin/workshop-practice
```
This creates a new (local) copy of the workshop-practice branch which "tracks" the online version of that branch (allows you to push changes to it and pull changes from it). 
...
* In RStudio, go to Tools>Version Control>Diff (or click Diff in the Git window), you will be able to see the differences between the local version of the file and the remote version, i.e. you'll be able to see the changes you have made (green lines have been added, red lines deleted).
...
* You can either do this from GitHub and use the same commands in the RStudio Terminal to create the branch, OR you can create the local and remote branch at the same time by typing the following into the terminal:
```markdown
