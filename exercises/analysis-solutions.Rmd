...
This notebook forms our analysis which will be `knitr`-ed into a `pdf` and then uploaded to  `osf.io`.  It comprises three four sections: 
...
3. *Baseline comparison - optional*: use `group_by` and `summarise` to compare groups. 
...
# Baseline characterisation - optional
...
```{r}
df <- select(df_tidy, -c(sex, agegroup))
head(df)
```
